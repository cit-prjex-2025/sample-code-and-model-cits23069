#include "app.h"
#include "util.h"
#include "timer.h" // タイマーのヘッダをインクルード
#include "horn.h"  // ホーンのヘッダをインクルード
...
// 荷物の積載・荷降ろし後の待機時間 (ミリ秒)
#define CARGO_WAIT_TIME 2000
...
void porter_transport(void) {
  ...
  case P_WAIT_FOR_LOADING:
    ...
    // 荷物が積まれ、タイマーが動いていない場合
    if( carrier_cargo_is_loaded() && !timer_is_started() ) {
        horn_confirmation(); // 確認音を鳴らす
        timer_start(CARGO_WAIT_TIME); // 2秒タイマーを開始
    }
    // タイマー中に荷物が降ろされた場合
    else if ( !carrier_cargo_is_loaded() && timer_is_started() ) {
        timer_stop(); // タイマーをキャンセル
    }
    // 荷物が積まれた状態でタイムアウトした場合
    else if ( carrier_cargo_is_loaded() && timer_is_timedout() ) {
        timer_stop(); // タイマーを停止
